...
apiRouter.get("/auth/profile", async (req, res) => {
  try {
    ...
    try {
      const { data: dbProfile } = await client.from("profiles").select("*").eq("id", user.id).maybeSingle();
...
apiRouter.get(["/watchlist", "/library"], async (req, res) => {
  try {
    const auth = await getAuthenticatedUser(req);
    if (auth.error || !auth.user || !auth.client) {
      return res.status(auth.status).json({
    ...
    const { data, error } = await auth.client.from("saved_items").select("*").eq("user_id", auth.user.id).order("created_at", { ascending: false });
    if (error) {
      return res.status(500).json({
...
apiRouter.post(["/watchlist", "/library"], async (req, res) => {
  try {
    const auth = await getAuthenticatedUser(req);
    if (auth.error || !auth.user || !auth.client) {
      return res.status(auth.status).json({
    ...
    const { data, error } = await auth.client.from("saved_items").upsert(row, { onConflict: "user_id,media_id" }).select().maybeSingle();
    if (error) {
      return res.status(500).json({
        error: "Database Error",
        message: error.message
...
apiRouter.delete(["/watchlist/:mediaId", "/library/:mediaId"], async (req, res) => {
  try {
    const auth = await getAuthenticatedUser(req);
    if (auth.error || !auth.user || !auth.client) {
      return res.status(auth.status).json({
    ...
    const { error } = await auth.client.from("saved_items").delete().eq("user_id", auth.user.id).eq("media_id", mediaId);
    if (error) {
      return res.status(500).json({
        error: "Database Error",
        message: error.message
